...
- [x] [Auto] Add and run repeatable real-evdev hardware-input validation: `scripts/verify-evemu-hardware-input.sh` requires an explicit `LWM_EVEMU_DEVICE` and latest run passed against `/dev/input/event6` with 60 `evemu-event` relative-motion commands, 60 hardware-cursor fast-path moves, zero fallback/unavailable moves, hardware presentation feedback, and zero fatal matches.
