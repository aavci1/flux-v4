...
- [x] [Auto] Add and run multi-dirty partial-damage and glass-chrome verification: latest run kept two decorated checker windows mapped while scripted multi-toplevel motion ran under a forced glass config, observed 411 partial frames, 332 multi-rect partial frames, 623 flips with at least two surfaces, 1,577 prepared blur ops / 1,574 blur runs, and kept surface avg/max at 0.059/0.086 ms under the verifier's 4.000 ms gate.
...
- [ ] [Manual] Complete the remaining visual checks from the plan: manual cursor appearance, resize/drag visual checks, and representative app inspection requiring interactive input. Current toolset has `evemu-event`, `wayland-info`, `ydotool`, and `wtype`, but this Codex process still has groups `aavci,nobody`, `/dev/uinput` and `/dev/input` are not visible, and `sudo -n true` is blocked by the container no-new-privileges flag, so uinput-backed validation cannot run here.
...
- [ ] [Manual] Complete remaining macOS runtime/visual verification for FP-14/FP-16 and latest Metal fixes: run `lambda-editor` with `LAMBDA_DEBUG_PERF=2`, paste large unicode-heavy text, inspect `CanvasDrawableWait`/atlas-grow behavior, and compare backdrop-blur visuals.
