...
- [ ] [Manual] Complete the remaining visual checks from the plan: manual cursor appearance, resize feel, and representative app inspection requiring interactive input. Captured drag/move stale-pixel checks now pass automatically, but this Codex process still has groups `aavci,nobody`, `/dev/uinput` and `/dev/input` are not visible, and `sudo -n true` is blocked by the container no-new-privileges flag, so live uinput-backed/manual validation cannot run here.
