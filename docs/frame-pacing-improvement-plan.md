...
- [x] Multi-dirty partial-damage, glass-chrome, and captured moving-window artifact verification is now enforced by `scripts/verify-frame-pacing-linux.sh` and `lambda-frame-artifact-check`. Latest run passed on 2026-06-11 (`.debug-logs/frame-pacing-verify/20260611-203118/multi-dirty-partial`): 432 partial frames, 345 multi-rect partial frames, max logical rect count 2, 632 flips with at least two surfaces, 1,275 prepared blur ops / 1,274 runs / 1.575 ms stacked blur work, zero fatal matches, surface avg/max 0.050/0.054 ms, 48 captured PNG frames, 555 expected checker-gradient content matches, and 573/579 newly exposed old-window samples returned to the solid background.
