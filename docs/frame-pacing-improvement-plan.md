...
- [x] Multi-dirty partial-damage and glass-chrome verification is now enforced by `scripts/verify-frame-pacing-linux.sh`. Latest run passed on 2026-06-11 (`.debug-logs/frame-pacing-verify/20260611-180528/multi-dirty-partial`): 411 partial frames, 332 multi-rect partial frames, max logical rect count 2, 623 flips with at least two surfaces, forced glass blur active with 1,577 prepared blur ops / 1,574 runs / 1.678 ms stacked blur work, zero fatal matches, and surface avg/max 0.059/0.086 ms under the 4.000 ms gate.
- [x] FP-8 offscreen scanout-copy validation is now enforced by `scripts/verify-frame-pacing-linux.sh`. Latest full run passed on 2026-06-11 (`.debug-logs/frame-pacing-verify/20260611-185636/scanout-copy-partial`) with direct scanout rendering disabled: 167 partial frames, 156 multi-rect partial frames, 128 region scanout copies, 793 scanout copy rects, region scanout-copy pixels at 64,051,854 / 265,420,800 full-equivalent pixels (0.241 ratio), primary-preserve region-copy ratio 0.209, and present avg/max 0.533/0.588 ms.
...
- [ ] Remaining local input/visual gap: `ydotool`, `wtype`, `evemu-event`, and `wayland-info` are installed, and real evdev input is validated. In this Codex session, the process still has groups `aavci,nobody`, `/dev/uinput` and `/dev/input` are not visible, and `sudo -n true` is blocked by the container no-new-privileges flag, so uinput-backed validation cannot run here. Manual cursor visual validation still requires an interactive human session.
...
- [x] [Auto] Hardware-cursor fast path remains active while a scripted terminal workload is presenting: latest run reported 180/180 hardware-cursor fast-path moves, 882 terminal present samples, zero pointer-triggered render loops, and 1,487.834 ms of verified timestamp overlap.
...
- [ ] [Manual] Interactive Wayland resize and animation demos hold refresh rate on a FIFO-only driver (e.g. lavapipe window).
