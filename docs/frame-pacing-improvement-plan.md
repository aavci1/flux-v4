...
- [x] FP-6 native Wayland popover pacing is now enforced by `scripts/verify-wayland-popover-pacing.sh`. Latest run passed on 2026-06-11 (`.debug-logs/wayland-popover-pacing/20260611-211815`) under headless Weston GL: one immediate initial configure render, eight committed redraw requests, eight frame requests, eight frame-done callbacks, eight committed renders from frame callbacks, and zero immediate committed redraw renders.
- [ ] Remaining local input/visual gap: `ydotool`, `wtype`, `evemu-event`, and `wayland-info` are installed, and real evdev input is validated. Outside the sandbox the user is now in `wheel` and `/dev/input/event*` is visible with ACLs, but `/dev/uinput` is still `0600 root:root` and `sudo -n true` still requires a password, so uinput-backed validation cannot run non-interactively here. Manual cursor visual validation still requires an interactive human session.
- [ ] Remaining environment gap: automated app smoke passed with `lambda-shell`, scripted `lambda-terminal`, and `lambda-editor`, and captured drag/move stale-pixel checks now pass; manual visual checks are still needed for cursor appearance, resize feel, and representative app inspection on a live compositor session.
...
- [x] [Auto] Verify popover first paint and committed-update pacing. `scripts/verify-wayland-popover-pacing.sh` runs `popover-demo` in headless autotest mode and requires the first configured paint to render immediately, committed reactive updates to render from `wl_surface_frame` callbacks, and zero immediate committed redraw renders. Latest run passed on 2026-06-11 (`.debug-logs/wayland-popover-pacing/20260611-211815`): initial_render=1, committed_redraw=8, frame_request=8, frame_done=8, paced_render=8, immediate_committed_render=0.
...
- [ ] [Manual] VT switch and mode changes still work.
