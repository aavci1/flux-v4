...
- **Configuration file:** `~/.config/flux-compositor/config.toml`. Keybindings, window-management preferences, animation toggles. Initial parsing and hot reload support `background = "#RRGGBB"`, `background_gradient = "#RRGGBB,#RRGGBB"`, `wallpaper = "/path/to/file.webp"`, `wallpaper_mode = "cover"` (`cover`, `contain`, `stretch`, `center`, or `tile`), `animations = false`, and a `[keybindings]` section for compositor shortcuts; `FLUX_COMPOSITOR_CONFIG=/path/to/config.toml` can override the path for testing.
