...
- **Configuration file:** `~/.config/flux-compositor/config.toml`. Keybindings, window-management preferences, animation toggles. Hot-reload optional. Initial startup parsing supports `background = "#RRGGBB"`, `background_gradient = "#RRGGBB,#RRGGBB"`, `wallpaper = "/path/to/file.webp"`, `wallpaper_mode = "cover"` (`cover`, `contain`, `stretch`, `center`, or `tile`), `animations = false`, and a `[keybindings]` section for compositor shortcuts; `FLUX_COMPOSITOR_CONFIG=/path/to/config.toml` can override the path for testing.
...
- ◐ Configuration via the config file works for keybindings and basic preferences. Startup background color, animation-toggle parsing, and compositor keybinding overrides are implemented; hot reload remains pending.
...
| Phase 5: Animation + polish | Initial polish in progress | 2026-05-17 | - | New toplevels fade/scale in, closed surfaces fade out, snap/maximize geometry changes animate through intermediate client configures, titlebar drag-to-edge shows a snap preview, and config can set the startup background color/gradient/image, disable animations, or override compositor shortcuts; interactive move/resize smoothing, hot-reload config, hardware cursor, frame timing, and docs remain pending. |
