...
- `zwlr_layer_shell_v1` exists and is used as the first transport for shell surfaces. Reserved work area and command-launcher keyboard modality follow client `set_exclusive_zone`, anchor, and `keyboard_interactivity` — not `lambda.*` namespace strings. Chrome appearance uses `xx_layer_chrome_v1` (or client blur regions), not namespace-based rendering policy.
...
The existing `zwlr_layer_shell_v1` implementation is enough to start rendering top-bar and dock surfaces. `lambda-window-manager` aggregates exclusive zones from all layer surfaces (top-anchored with positive exclusive zone reserves the top edge; bottom-anchored surfaces contribute dock margin). Command-launcher modality uses `keyboard_interactivity == exclusive`. Shell-surface authentication remains future work.
