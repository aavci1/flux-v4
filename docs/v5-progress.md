...
- Environment hard cutover: type-keyed `std::any` environment layers were replaced with compile-time environment keys, flat keyed bindings, keyed `.environment<Key>(...)` modifiers, and signal-shaped `useEnvironment<Key>()` helpers.
