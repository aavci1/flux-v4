...
| P21 | WM-COMP-22 XDG toplevel configure-state parity | Verified slice | `ack_configure` serial consumption, pending-to-current configure commit, pending-configure frame-size selection, window-manager transition geometry, XDG state configure sizing, popup parent geometry, cutout rejection sizing, client resize-grab starts, and top-level pointer hit bounds now use explicit tested helpers; remaining low-level consumer audit continues | XDG surface/window-state tests plus compositor suite pass | No manual gate for these helper slices |
...
1. In progress: compared Lambda's `configureList`, `pendingConfigure`, and `currentConfigure` fields and extracted the first ack/commit transition helper slice.
2. In progress: identified a concrete snapshot/chrome hit-testing split where snapshots froze pending-configure frame size to committed dimensions while chrome hit testing still used live display dimensions.
3. In progress: moved that frame-size decision into a shared helper used by snapshots, top chrome hit testing, cutout controls hit testing, `windowGeometryFor`, window-manager transition start-size bookkeeping, XDG state configure sizing, popup parent geometry, initial/cutout rejection sizing, client resize-grab starts, and top-level pointer hit bounds. Remaining lower-level layer/subsurface/snapshot-local consumers still need the final audit before closing WM-COMP-22.
4. In progress: added automated tests for `ack_configure` serial consumption, stale configure pruning, resize-configure detection, pending-to-current configure commit, pending-configure interactive frame-size selection, committed-size availability, and window geometry helper coherence. Broader resize/maximize/fullscreen/chrome consumer tests remain planned.
5. Not needed for the helper slices so far: no visible frame timing behavior changed.
...
3. Planned: compare the remaining wlroots seat focus and grab lifecycle with Lambda's non-popup pointer, keyboard, cursor, and layer-shell input paths.
