...
| P26 | WM-COMP-27 Visual regression and real-app harness | In progress | Added the real-app validation matrix, a drift checker, mpv coverage, an opt-in smoke-runner mode that starts/stops `lambda-window-manager` with CPU/KMS/pacing traces, `wayland-info` protocol registry validation, source/list drift checks against advertised globals, fatal/protocol log scanning, and a remaining-gate audit helper | `verify-wm-comp27-matrix.sh`, smoke-runner syntax/list/probe checks, audit-helper checks, and start-compositor guard checks pass | Target hardware visual gate remains |
