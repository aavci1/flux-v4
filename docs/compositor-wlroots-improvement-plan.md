...
| P8 | WM-COMP-9 XDG toplevel request and configure parity | Verified slice | XDG toplevel titles now use wlroots-style UTF-8 validation; app_id remains opaque like wlroots | XDG toplevel/title tests plus compositor suite pass | No manual gate needed for this protocol-validation slice |
...
4. Planned: compare xdg_toplevel parent mapped/unmapped semantics against wlroots.
5. Planned: compare scheduled/pending/current toplevel configure state against wlroots and decide whether Lambda needs a larger synced-state migration after the deferred titlebar/content issue.
