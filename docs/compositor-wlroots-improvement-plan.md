...
| P3 | WM-COMP-4 Scene and output damage architecture | In progress | Damage-aware atomic dirty-frame skip validated on target hardware; next step is frame-callback visibility correctness | Snapshot/damage tests plus render scheduler tests | DP-1 resize trace, real-app flicker check, video/browser pacing |
| P4 | WM-COMP-5 Seat serials and grab model | Planned | Starts after WM-COMP-4 reaches its callback-correctness gate | Seat serial, popup grab, selection, and focus tests | Popups, drag/drop, clipboard, and menu behavior in real apps |
...
4. Next: ensure frame callbacks are sent only when a committed visible surface participates in the frame.
