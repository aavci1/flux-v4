...
| P4 | WM-COMP-5 Seat serials and grab model | In progress | Xdg move/resize/menu and popup grab serial validation now use the seat ledger; next step is data-device and selection validation | Seat serial, popup grab, selection, and focus tests | Popups, drag/drop, clipboard, and menu behavior in real apps |
...
3. Partially done: xdg popup grabs and xdg move/resize/menu requests now use the shared serial validation model. Next route data-device requests and primary selection through the same model.
