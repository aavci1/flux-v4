...
| P25 | WM-COMP-26 Output layout and multi-output foundation | In progress | Added tested selected-output layout and output-enter policy helpers; `wl_output.geometry`, surface enter on output bind/surface creation, and `xdg_output.logical_position/size` now consume shared policy while staying single-output for v1 | Output/xdg-output/snapshot tests plus compositor suite | Target hardware multi-output gate before enabling multiple active outputs |
...
1. In progress: compare wlroots output-layout responsibilities with Lambda's selected-output runtime.
...
3. In progress: add tests for output enter/leave selection, xdg-output logical position/size calculation, and layer/window placement on a logical layout. Current slices cover xdg-output logical position/size updates and single-output surface-enter policy.
