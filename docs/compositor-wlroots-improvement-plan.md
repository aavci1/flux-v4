...
| P24 | WM-COMP-25 Layer-shell dynamic behavior parity | In progress | Keyboard interactivity, dynamic committed layer/reserved-zone state, anchor/margin placement, layer-surface popup reparenting, and fullscreen dock-hide policy are covered by tested helpers; remaining output changes, focus, and output-scale updates are planned | Layer-shell tests plus compositor suite | Shell dock/topbar/fullscreen validation if placement/focus behavior changes |
...
4. Verified slice: added committed-state coverage for dynamic layer changes and dock reserved-zone anchor/margin aggregation, moved anchor/margin placement into a tested helper used by runtime layer geometry, and centralized the fullscreen dock-hide policy used by input routing and frame-callback filtering. Remaining focus, output-scale updates, visual fullscreen validation, and deeper placement scenarios are planned.
