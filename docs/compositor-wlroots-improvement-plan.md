...
| P21 | WM-COMP-22 XDG toplevel configure-state parity | Verified slice | `ack_configure` serial consumption and pending-to-current configure commit are now explicit tested helpers; renderer/chrome consumer audit continues | XDG surface/window-state tests plus compositor suite pass | No manual gate for this non-visual state-helper slice |
...
4. In progress: added automated tests for `ack_configure` serial consumption, stale configure pruning, resize-configure detection, and pending-to-current configure commit. Broader resize/maximize/fullscreen/chrome consumer tests remain planned.
5. Not needed for the first helper slice: no visible frame timing behavior changed.
