...
| P23 | WM-COMP-24 Data-device and drag/drop lifecycle parity | In progress | DnD action-mask, preferred-action, and selected-action negotiation rules are factored into a tested helper; source/offer lifecycle comparison remains planned | Data-device/DnD/selection tests plus compositor suite | Manual DnD/file-transfer validation if payload behavior changes |
