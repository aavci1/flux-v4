...
| P23 | WM-COMP-24 Data-device and drag/drop lifecycle parity | In progress | DnD action negotiation and destination-side offer request validation are factored into tested helpers; source/offer lifecycle comparison remains planned | Data-device/DnD/selection tests plus compositor suite | Manual DnD/file-transfer validation if payload behavior changes |
...
6. Planned: request manual validation if file payloads, external app DnD, or visible drag icons are changed.
