...
| P20 | WM-COMP-21 Remaining global resource hygiene | Verified | Core, shm, layer-shell, cutouts, xdg-shell/decoration, linux-dmabuf, background-effect, activation, primary-selection, data-device, and pointer-extension manager paths now have explicit version caps or no-memory bind/object guards; global advertisement now uses the same named version constants; existing dependent-resource cleanup was audited | `CompositorGlobalResourceHygieneTests.cpp`, focused protocol helper tests, compositor source-file suite, and build pass | No manual gate needed for this protocol-resource slice |
...
| P22 | WM-COMP-23 Seat focus and grab workflow parity | Verified slices | xdg-popup grabs now follow the wlroots popup-grab model for the implemented pointer/keyboard seat paths; unmapped XDG popup/toplevel surfaces plus direct `wl_surface.destroy` and destroyed XDG popup/toplevel/layer-shell roles now clear stale seat focus/order/grab/serial/activation state while deactivating pointer constraints; layer-shell null-buffer unmap now clears stale command-launcher modal focus through the same seat cleanup path; cursor requests, cursor-shape device surface-destroy lifetime, popup-parent unmap/role-destroy cancellation, non-popup implicit pointer-button grabs including motion focus pinning, popup-grab keyboard-focus pin/restore decisions, keyboard Escape popup-grab stack cancellation, unsupported-touch seat resource handling, stale popup-grab seat-resource cleanup, data-device/primary-selection device seat-resource cleanup, and native Wayland popover Escape dismissal now use tested paths. Broader non-popover live-resource keyboard/menu workflow tests remain planned. | Seat serial, pointer-button grab, popup, keyboard-focus, layer-shell, window-state, pointer-constraint, cursor-shape, global-resource, config, compositor, data-device, popover pacing, and broader feasible tests pass | Firefox app/context menu actions and click-open submenus passed; no manual gate needed for these cleanup slices |
| P23 | WM-COMP-24 Data-device and drag/drop lifecycle parity | Verified | DnD action negotiation, offer/source validation, single-use source lifecycle, drag-icon role validation, post-finish offer request rejection, completed-drop cleanup, post-drop offer-abort cancellation, compositor-cancel source notification, and selection/DnD serial policies are factored into tested helpers | Data-device/DnD/selection tests plus compositor suite pass | No manual gate needed for these protocol-state slices; file payloads, external app DnD, and visible drag icons were not changed |
| P24 | WM-COMP-25 Layer-shell dynamic behavior parity | Verified | Keyboard interactivity, dynamic committed layer/reserved-zone state, anchor/margin placement, layer-surface popup reparenting, fullscreen dock-hide policy, command-launcher modal focus policy, and output-geometry reconfigure sequencing are covered by tested helpers | Layer-shell tests plus compositor suite pass | No manual gate needed for these helper/state slices; visual Shell validation remains WM-COMP-27 |
| P25 | WM-COMP-26 Output layout and multi-output foundation | Verified | Selected-output layout now drives `wl_output.geometry`, `xdg_output.logical_position/size`, output-enter selection, layer-shell placement, and window snap/maximize/resize/popup geometry while v1 keeps one active output | Output/xdg-output/layer-shell/window-geometry tests plus compositor suite pass | Full multi-output desktop remains deferred until product and target-hardware validation |
| P26 | WM-COMP-27 Visual regression and real-app harness | In progress | Added the real-app validation matrix, a drift checker, mpv coverage, grouped smoke-runner case selection, an opt-in smoke-runner mode that starts/stops `lambda-window-manager` with CPU/KMS/pacing traces, `wayland-info` protocol registry validation, source/list drift checks against advertised globals, fatal/protocol log scanning, and a remaining-gate audit helper | `verify-wm-comp27-matrix.sh`, smoke-runner syntax/list/grouped-probe checks, audit-helper checks, and live core Lambda app smoke pass | Target hardware visual gate remains |

## Remaining Work After 2026-06-01

Default next implementation order is the remaining non-popup WM-COMP-23 seat/grab parity and WM-COMP-27 target-hardware validation, unless the deferred WM-COMP-1 titlebar/content sync issue is explicitly resumed first.
...
- Compare larger behavior workflows against wlroots: remaining seat focus/grabs and repeatable real-app visual validation.
...
8. Verified slice: unsupported `wl_seat.get_touch` now creates an inert `wl_touch` resource with normal release/destruction instead of ignoring the request, while advertised seat capabilities remain pointer plus keyboard only and seat bind no-memory is guarded.
9. Verified slice: non-popup implicit pointer-button grabs now pin pointer motion focus to the grabbed surface while buttons are held, then refresh focus to the current hit-test target after the final release.
...
8. Verified slice: non-popup implicit pointer-button grab lifecycle now uses a tested helper for first-press capture, focus-change delivery, nested button counts, final-release cleanup, and stale destroyed-surface cancellation while preserving the existing runtime trace path.
9. Verified slice: popup-grab keyboard-focus pinning, stale cached-grab-top cleanup, and popup-dismiss/grab-dismiss focus restoration predicates now use tested helpers.
10. Verified slice: `wl_seat` resource destruction now clears any xdg-popup grab stack that references the destroyed seat resource, preventing stale raw `wl_resource*` references in the popup grab state and cached top pointer.
11. Verified slice: `wl_seat` resource destruction now clears matching `wl_data_device` and `zwp_primary_selection_device_v1` stored seat-resource references, preventing stale raw `wl_resource*` metadata after clients release a seat resource.
12. Verified slice: cursor-shape device lifetime now follows the dependent `wl_pointer` resource only; destroying an unrelated same-client `wl_surface` no longer clears the cursor-shape device pointer.
13. Verified slice: XDG toplevel null-buffer unmap and role destruction now reset child `xdg_popup` objects parented to the toplevel surface, sending `popup_done` through the same cleanup path used for parent surface destruction.
14. Verified slice: keyboard-driven popup cancellation now follows the same active-grab-stack dismissal path as pointer-outside cancellation when popup grabs are enabled, while popup-grab-disabled mode keeps the top-popup dismissal behavior.
15. Verified slice: `scripts/verify-wayland-popover-pacing.sh` now keeps a native xdg-popup popover open through eight paced committed redraws, then dismisses it through Escape and requires the popover `onDismiss` path to run. The verifier uses `wtype` when the compositor advertises virtual-keyboard support and a guarded Linux Wayland backend Escape hook under Weston headless, which lacks that protocol.
16. Verified slice: layer-shell null-buffer unmap now runs the shared surface seat cleanup, so an unmapped command-launcher/modal layer cannot keep stale keyboard focus, pointer focus, pointer-button grabs, serials, activation-token references, or `commandLauncherModalSurface_`.
17. Verified slice: XDG popup null-buffer unmap now runs the shared surface seat cleanup after releasing any popup grab, so an unmapped popup cannot retain stale keyboard focus, pointer focus, pointer-button grabs, focus-cycle/order state, seat serials, or activation-token references.
18. Verified slice: direct `wl_surface.destroy` now routes seat focus/order/serial/activation cleanup through the shared unmapped-surface helper while preserving the existing pointer-button-grab diagnostic detach path, so surface destruction cannot miss cleanup fields added to the shared helper.
19. Planned: request manual app menu/dock menu validation if runtime grab behavior changes again.
...
1. Done: inventoried data-source, data-offer, data-device, drag icon, target enter/motion/leave/drop, and action negotiation behavior while implementing the verified helper slices below.
2. Done: compared cancellation and cleanup behavior for active source, offer, target surface/device, no-target/no-action drop, completed drop, and source self-destruction paths.
3. Verified slice: selection and DnD serial checks now use named shared serial-kind policies. DnD start accepts only pointer-button press serials on the origin surface; data-device and primary-selection `set_selection` accept keyboard enter/key or pointer-button press serials on keyboard focus.
...
11. Verified slice: compositor-cancel cleanup now notifies the active source through `wl_data_source.cancelled` for no-target/no-action drops, destroyed targets/devices, and replaced active DnD sessions, while source self-destruction suppresses the event.
12. Not needed for this protocol-state workstream: file payload handling, external app payload transfer, and visible drag icon rendering behavior were not changed.
...
1. Done: compared the automatable layer output/configure/map/unmap/exclusive-zone paths against Lambda's current single-output runtime and captured the resulting invariants in tested helpers. Multi-output layout remains WM-COMP-26.
2. Verified slice: keyboard interactivity behavior for none, exclusive, and on-demand modes is centralized in a tested helper. Pre-`on_demand` layer-shell resources still normalize any non-zero request to exclusive, v4 resources preserve `on_demand`, and invalid enum values are rejected.
3. Verified slice: layer-surface popup reparenting now uses a tested helper that preserves parent-relative popup coordinates when `get_popup` attaches an existing xdg-popup to a layer surface. Broader shell-owned menu interaction validation remains planned if runtime behavior changes.
4. Verified slice: added committed-state coverage for dynamic layer changes and dock reserved-zone anchor/margin aggregation, moved anchor/margin placement into a tested helper used by runtime layer geometry, centralized the fullscreen dock-hide policy used by input routing and frame-callback filtering, centralized the command-launcher modal focus policy, and gated output-geometry reconfigure on initialized layer resources.
5. Not needed for this helper/state pass: no intended visual placement/focus behavior changed. Repeatable Shell dock/topbar/fullscreen/menu visual validation is tracked by WM-COMP-27.
...
1. Done: compared wlroots output-layout responsibilities with Lambda's selected-output runtime and isolated the v1 single-output policy behind layout helpers.
2. Verified slice: separated the selected-output single-output policy from a tested `OutputLayoutBox` that carries logical position, effective logical size, scale, and transform.
3. Verified slice: added tests for output enter/leave selection, xdg-output logical position/size calculation, layer-shell placement, window snap/maximize/resize placement, and xdg-popup output constraints on logical layouts with non-zero origins.
4. Done: multiple active outputs remain disabled until there is an explicit product decision and target-hardware validation plan.
...
1. Verified slice: documented the current real-app validation matrix for Settings, Files, Editor, Terminal, Shell, Firefox/browser, GTK, Qt, `foot`, and mpv.
2. Verified slice: `scripts/run-real-app-smoke.sh` launches clients and collects logs against an already-running compositor, and now has an opt-in `--start-compositor` mode that starts `lambda-window-manager`, waits for its display file, runs selected cases, captures compositor stderr plus CPU/KMS/pacing traces, scans collected logs for fatal/protocol/runtime failures, and stops the compositor.
3. Verified slice: added `scripts/verify-wm-comp27-matrix.sh` so the documented matrix, explicit visual criteria, and smoke-runner case list stay in sync.
4. Verified slice: visual pass/fail criteria are explicit in the matrix: no flicker, no non-synced chrome/content frames after the deferred fix, dock/topbar stable, menus/popovers do not crash Shell or compositor, fullscreen panels restore, cursor updates immediately.
5. Verified slice: owned-compositor smoke runs now capture `wayland-info`, save a registry summary, and fail if required protocol globals such as `xdg_wm_base`, layer-shell, cursor-shape, viewporter, presentation-time, data-device, primary-selection, and activation are missing.
6. Verified slice: the registry verifier now compares the smoke-runner required-global list with the compositor's `wl_global_create` interfaces, covering dmabuf and `ext_background_effect_manager_v1` and preventing source/list drift.
7. Verified slice: the GTK smoke-runner case now recognizes `meld` and `pavucontrol` fallbacks in addition to GNOME Text Editor, gedit, and Nautilus, so this machine's installed GTK validation option is detected instead of reporting a false required-app gap.
8. Verified slice: added `scripts/audit-wm-comp27-validation-gates.sh` so the remaining real-app validation gates are reproducible. The audit reports required app/tool readiness, live Lambda Wayland session and registry availability, TTY/uinput/sudo/evdev prerequisites for target-hardware driving, helper presence, and the still-manual visual checks.
9. Verified slice: `scripts/run-real-app-smoke.sh --case` now accepts repeated and comma-separated case groups, preserving the existing all-cases and explicit Shell-launch behavior while allowing focused live validation batches such as core Lambda apps without also launching every external client.

**Real-app validation matrix:**

| Case | Smoke runner case | Required app/source | Primary workflows | Pass/fail criteria |
| --- | --- | --- | --- | --- |
| Settings | `lambda-settings` | `lambda-settings` | Launch, focus, move, resize, snap, maximize/restore, settings page navigation, text input | No flicker, no non-synced chrome/content frames, cursor updates immediately, close is clean |
| Files | `lambda-files` | `lambda-files` | Browse folders, integrated titlebar, context menus, selection, default open, resize/snap | No stretched or stale content, menus/popovers do not crash Shell or compositor, text/icons remain visible |
| Editor | `lambda-editor` | `lambda-editor` | Open a text file, scroll beyond initial height, toolbar tooltips, find panel, resize/snap | Text keeps rendering after scroll, no empty text region, no stale content during resize |
| Terminal | `lambda-terminal` | `lambda-terminal` | Shell prompt, typing, Ctrl+C delivery to child process, scrollback, resize | Resize feels continuous, cursor stays responsive, no stretched terminal contents |
| Shell | `shell` | running or launched `lambda-shell` | Dock, launcher, dock context menus, status docklets, focus/restore | Dock/topbar stable, menus/popovers do not crash Shell or compositor, fullscreen panels restore |
| Firefox | `browser` | Firefox preferred, Chromium/Chrome fallback | Native Wayland launch, address-bar typing, scrolling, menus/popovers, copy/paste | Menus stay interactive, no compositor crash, no stale popup/focus state |
| GTK | `gtk` | `gnome-text-editor`, `gedit`, `nautilus`, `meld`, or `pavucontrol` | Text input, file browsing, or GTK controls; headerbar menus/popovers, clipboard where applicable, resize/snap | GTK popovers remain stable, resize has no stretched/stale content |
| Qt | `qt` | `kate`, `dolphin`, or `qterminal` | Menus/popovers, text input or file browsing, clipboard, resize/snap | Qt menus remain stable, no focus loss or compositor crash |
| foot | `terminal` | `foot` preferred, `qterminal` fallback | Mature Wayland terminal typing, Ctrl+C, scrollback/reflow, resize | Terminal input remains client-owned, resize remains continuous |
| mpv | `mpv` | `mpv` | Wayland player window, fullscreen toggle where practical, video/player chrome, close | Video/player surface stays stable, fullscreen panels restore, cursor updates immediately |
...
| 2026-06-12 | WM-COMP-21 | Verified slice | Extended the tested global resource-version helper pattern to activation, primary-selection, data-device, and relative-pointer manager paths. Activation token, primary-selection source/device, data-source, data-device, relative-pointer, and pointer-constraint child resources now derive their resource versions through explicit helper caps instead of local `std::min` or hardcoded values. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --source-file='*CompositorGlobalResourceHygieneTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorPointerExtensionTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorDataDeviceTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorWindowStateTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, and `git diff --check` passed. |
| 2026-06-12 | WM-COMP-21 | Verified slice | Extended explicit resource-version helpers to the remaining xdg-shell, xdg-decoration, linux-dmabuf, and background-effect bind/object paths. XDG positioner/surface/toplevel/popup, toplevel-decoration, dmabuf params/feedback, and background-effect surface resources now inherit capped manager-bound versions through named helpers instead of raw `wl_resource_get_version()` or local `std::min`. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --source-file='*CompositorGlobalResourceHygieneTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorWindowStateTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorDecorationStateTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorDmabuf*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, and `git diff --check` passed. |
| 2026-06-12 | WM-COMP-21 | Verified slice | Routed Wayland global advertisement in `ServerLifecycle.cpp` through the same named protocol version constants used by bind/object resource creation. This removes the remaining numeric global-version table from lifecycle setup and keeps advertised versions tied to the tested caps. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --source-file='*CompositorGlobalResourceHygieneTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, the `wl_global_create` numeric-version scan returned no matches, and `git diff --check` passed. |
...
| 2026-06-12 | WM-COMP-23 | Verified slice | Factored non-popup implicit pointer-button grab lifecycle into `PointerButtonGrabState.hpp` and routed `sendPointerButtonToFocus()` through it. Tests now pin that first press captures the delivery surface/client, focus changes continue delivering to the grabbed surface/client, nested button counts hold the grab until final release, and stale destroyed-surface grabs clear count/client before fallback delivery. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --source-file='*CompositorSeatSerialTests.cpp' --no-skip`, `./build/tests/lambda_tests --test-case='*pointer button grab*' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorWindowStateTests.cpp' --no-skip`, `./build/tests/lambda_tests --test-case='*popup grab*' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, and `git diff --check` passed. |
| 2026-06-12 | WM-COMP-23 | Verified slice | Factored popup-grab keyboard-focus decisions into `SeatFocusState.hpp` and routed `setKeyboardFocus()`, popup dismiss, and popup-grab dismiss through them. Tests now pin that active popup grabs force keyboard focus to the top grabbed popup, disabled popup grabs pass requested focus through, dismissed popups are not focus targets, empty grabs clear stale cached top pointers, and popup/grab dismissal restores toplevel focus only when keyboard focus was on a popup. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --test-case='*keyboard focus helper*,*keyboard focus restoration*' --no-skip`, `./build/tests/lambda_tests --test-case='*popup grab*,*popup focus*,*unmapped surface cleanup*,*seat serial*' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorWindowStateTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorSeatSerialTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, and `git diff --check` passed. |
| 2026-06-12 | WM-COMP-23 | Verified slice | Added inert `wl_touch` resource handling for clients that call `wl_seat.get_touch` even though Lambda advertises only pointer and keyboard capabilities. `bindSeat()` now guards seat-resource allocation failure, pointer/keyboard/touch child resources share the explicit seat resource-version cap, and touch resources are tracked until release/destruction without advertising touch capability. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --source-file='*CompositorGlobalResourceHygieneTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorSeatSerialTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, and `git diff --check` passed. |
| 2026-06-12 | WM-COMP-23 | Verified slice | Extended the non-popup implicit pointer-button grab helper to pointer motion focus. While any pointer button is held, motion/position focus remains pinned to the grabbed surface even if hit-testing moves over another surface; after the final release, pointer focus refreshes to the current hit-test target. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --source-file='*CompositorSeatSerialTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorWindowStateTests.cpp' --no-skip`, `./build/tests/lambda_tests --test-case='*pointer button grab*' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, and `git diff --check` passed. |
| 2026-06-12 | WM-COMP-23 | Verified slice | Cleared xdg-popup grab stacks when the `wl_seat` resource that authorized the grab is destroyed, so popup grab state and cached grab-top pointers cannot retain stale seat resource pointers. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --source-file='*CompositorWindowStateTests.cpp' --no-skip`, `./build/tests/lambda_tests --test-case='*popup grab*' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorSeatSerialTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, and `git diff --check` passed. |
| 2026-06-12 | WM-COMP-23 | Verified slice | Cleared stored seat-resource references from `wl_data_device` and primary-selection device state when the originating `wl_seat` resource is destroyed. The data/primary-selection devices remain inertly valid, but no longer retain stale raw seat resource pointers. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --source-file='*CompositorDataDeviceTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorGlobalResourceHygieneTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorSeatSerialTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, and `git diff --check` passed. |
| 2026-06-12 | WM-COMP-23 | Verified slice | Preserved cursor-shape devices across unrelated same-client surface destruction. Cursor-shape device lifetime now follows the dependent `wl_pointer` resource only, while pointer-resource destruction still removes dependent cursor-shape devices. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --source-file='*CompositorCursorShapeTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorSeatSerialTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorWindowStateTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, and `git diff --check` passed. |
| 2026-06-12 | WM-COMP-23 | Verified slice | Reset child `xdg_popup` objects when an XDG toplevel parent is unmapped with a null buffer or when the toplevel role is destroyed. Popup parent cancellation now uses the same `popup_done` cleanup path as parent surface destruction, preventing child popups from retaining a parent surface whose XDG role has gone away. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --source-file='*CompositorWindowStateTests.cpp' --no-skip`, `./build/tests/lambda_tests --test-case='*popup*' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorSeatSerialTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, and `git diff --check` passed. |
| 2026-06-12 | WM-COMP-23 | Verified slice | Routed keyboard-triggered popup dismissal through the active popup-grab stack cancellation path when popup grabs are enabled. Escape now clears the same grabbed popup stack as pointer-outside dismissal, while popup-grab-disabled mode keeps the top-popup fallback behavior; the decision is covered by `keyboardDismissShouldClearPopupGrab()`. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --source-file='*CompositorWindowStateTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorSeatSerialTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, and `git diff --check` passed. |
| 2026-06-13 | WM-COMP-23 | Verified slice | Extended the headless native Wayland popover verifier into the live-resource keyboard-dismissal gap. `popover-demo` autotest now waits for Escape dismissal through the native popover `onDismiss` path after eight committed redraws; `scripts/verify-wayland-popover-pacing.sh` uses `wtype` on compositors with virtual-keyboard support and a guarded Linux Wayland backend Escape hook under Weston headless, which does not advertise that protocol. Build passed for `lambda_tests` and `popover-demo`; `scripts/verify-wayland-popover-pacing.sh` passed with `escape_driver=backend`, initial_render=1, committed_redraw=8, frame_request=8, frame_done=8, paced_render=8, immediate_committed_render=0, autotest_dismissed=1, backend_escape=1, and `git diff --check` passed. |
| 2026-06-13 | WM-COMP-23 | Verified slice | Routed layer-shell null-buffer unmap through the shared surface seat cleanup path and made that helper clear `commandLauncherModalSurface_`, so a hidden command-launcher/modal layer cannot retain stale keyboard focus, pointer focus, pointer-button grabs, serials, activation-token references, or modal state after unmap. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --source-file='*CompositorWindowStateTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorLayerShellZoneTests.cpp' --no-skip`, `./build/tests/lambda_tests --test-case='*unmapped surface cleanup*,*command launcher modal focus policy*,*layer shell map and unmap reset configure state*' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorSeatSerialTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, RuntimeInput under headless Weston, full headless-Weston `ctest`, `scripts/verify-wayland-popover-pacing.sh`, and `git diff --check` passed. |
| 2026-06-13 | WM-COMP-23 | Verified slice | Routed XDG popup null-buffer unmap through the shared surface seat cleanup path after releasing any popup grab. Unmapped popups now clear stale keyboard focus, pointer focus, pointer-button grabs, focus order/cycle state, seat serial records, and activation-token surface references, and pointer constraints are refreshed when pointer focus is removed. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --source-file='*CompositorWindowStateTests.cpp' --no-skip`, `./build/tests/lambda_tests --test-case='*popup*' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorSeatSerialTests.cpp' --no-skip`, `./build/tests/lambda_tests --test-case='*pointer constraint*' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, RuntimeInput under headless Weston, full headless-Weston `ctest`, `scripts/verify-wayland-popover-pacing.sh`, and `git diff --check` passed. |
| 2026-06-13 | WM-COMP-23 | Verified slice | Routed direct `wl_surface.destroy` through the shared unmapped-surface seat cleanup helper while preserving the existing pointer-button-grab diagnostic detach path. Surface destruction now shares cleanup for focus order/cycle state, pointer focus, keyboard focus, command-launcher modal state, seat serial records, and activation-token surface references, and pointer constraints are refreshed when pointer focus is removed. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --source-file='*CompositorWindowStateTests.cpp' --no-skip`, `./build/tests/lambda_tests --test-case='*unmapped surface cleanup*,*seat serial*' --no-skip`, `./build/tests/lambda_tests --test-case='*pointer constraint*' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorSeatSerialTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, RuntimeInput under headless Weston, full headless-Weston `ctest`, `scripts/verify-wayland-popover-pacing.sh`, and `git diff --check` passed. |
...
| 2026-06-12 | WM-COMP-24 | Verified slice | Centralized compositor-cancel source notification in the DnD cleanup plan. No-target/no-action drops, destroyed targets/devices, and replaced active DnD sessions now send `wl_data_source.cancelled`; completed drops and source self-destruction suppress cancellation. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --source-file='*CompositorDataDeviceTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorSeatSerialTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, and `git diff --check` passed. |
| 2026-06-12 | WM-COMP-24 | Verified | Moved data-device drag-start and selection serial-kind policies into shared tested helpers. DnD start remains limited to pointer-button press serials on the origin surface; data-device and primary-selection `set_selection` accept keyboard enter/key and pointer-button press serials through the shared seat ledger. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --source-file='*CompositorSeatSerialTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorDataDeviceTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, and `git diff --check` passed. |
| 2026-06-12 | WM-COMP-25 | Verified slice | Factored layer-shell keyboard interactivity normalization/validation into tested helpers. Legacy resources normalize non-zero values to exclusive, v4 resources preserve `on_demand`, and invalid enum values are rejected before mutating pending state. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --test-case='*layer*' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorLayerShellZoneTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, and `git diff --check` passed. |
| 2026-06-12 | WM-COMP-25 | Verified slice | Added dynamic layer-shell committed-state coverage: dock reserved-zone aggregation now has explicit committed anchor/margin/max-extent tests, and pending `set_layer` changes are verified to apply only on commit without forcing a configure. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --source-file='*CompositorLayerShellZoneTests.cpp' --no-skip`, `./build/tests/lambda_tests --test-case='*layer*' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, and `git diff --check` passed. |
| 2026-06-12 | WM-COMP-25 | Verified slice | Moved layer-shell anchor/margin placement into `resolveLayerShellPlacement()` and routed runtime layer geometry through it. Tests now cover top-left, bottom-right, centered, and opposing-anchor placement behavior without requiring a live Wayland resource. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --source-file='*CompositorLayerShellZoneTests.cpp' --no-skip`, `./build/tests/lambda_tests --test-case='*layer*' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, and `git diff --check` passed. |
| 2026-06-12 | WM-COMP-25 | Verified slice | Factored xdg-popup reparenting for `layer_surface.get_popup` into a tested helper. Existing layer-surface popup attachment behavior now explicitly preserves the popup's parent-relative coordinates when moving from no parent or an old parent to the layer surface. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --source-file='*CompositorWindowStateTests.cpp' --no-skip`, `./build/tests/lambda_tests --test-case='*popup layer-surface reparenting*' --no-skip`, `./build/tests/lambda_tests --test-case='*layer*' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, and `git diff --check` passed. |
| 2026-06-12 | WM-COMP-25 | Verified slice | Centralized the fullscreen dock-hide layer-shell policy used by above-window input routing and frame-callback filtering. Tests now pin that only the `lambda.dock` layer namespace hides for fullscreen and that frame callbacks are filtered only after the hide animation reaches the existing completion threshold. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --source-file='*CompositorLayerShellZoneTests.cpp' --no-skip`, `./build/tests/lambda_tests --test-case='*fullscreen hide policy*' --no-skip`, `./build/tests/lambda_tests --test-case='*layer*' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, and `git diff --check` passed. |
| 2026-06-12 | WM-COMP-25 | Verified slice | Centralized the command-launcher modal focus policy used by `claimCommandLauncherModal()`. Tests now pin that only a mapped `lambda.command-launcher` layer surface with exclusive keyboard interactivity can claim modal focus; on-demand, none, unmapped, and other namespaces are rejected. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --source-file='*CompositorLayerShellZoneTests.cpp' --no-skip`, `./build/tests/lambda_tests --test-case='*command launcher modal focus policy*' --no-skip`, `./build/tests/lambda_tests --test-case='*layer*' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, and `git diff --check` passed. |
| 2026-06-12 | WM-COMP-25 | Verified | Gated layer-shell output-geometry reconfigure on initialized layer resources, so runtime output-scale changes cannot send configure events before the required initial empty commit or after dependent surface/resource cleanup. This completes the automatable WM-COMP-25 helper/state pass; repeatable visual Shell validation continues under WM-COMP-27. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --source-file='*CompositorLayerShellZoneTests.cpp' --no-skip`, `./build/tests/lambda_tests --test-case='*output geometry reconfigure*' --no-skip`, `./build/tests/lambda_tests --test-case='*layer*' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, and `git diff --check` passed. |
| 2026-06-12 | WM-COMP-26 | Verified slice | Added a tested `OutputLayoutBox` for the selected output. It preserves current single-output origin behavior while carrying logical position, logical size, scale, and transform for future layout work; `logicalOutputWidth/Height`, `wl_output.geometry`, and `xdg_output.logical_position/size` now use it, and xdg-output update detection includes logical position. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --source-file='*CompositorOutputStateTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorXdgOutputTests.cpp' --no-skip`, `./build/tests/lambda_tests --test-case='*output layout*,*xdg output logical-geometry*' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, and `git diff --check` passed. |
| 2026-06-12 | WM-COMP-26 | Verified slice | Centralized the selected-output surface-enter policy used when a `wl_output` is bound and when a new `wl_surface` is created. Current v1 behavior remains active same-client output resources only, but the selection is now a tested helper ready for future layout intersection logic. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --source-file='*CompositorOutputStateTests.cpp' --no-skip`, `./build/tests/lambda_tests --test-case='*output enter selection*' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorXdgOutputTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, and `git diff --check` passed. |
| 2026-06-12 | WM-COMP-26 | Verified slice | Routed layer-shell placement through the selected output layout instead of `logicalOutputWidth/Height()` alone. `resolveLayerShellPlacement()` now accepts logical output origin plus size, preserving current zero-origin behavior while covering non-zero-origin top/left, right/bottom, and centered placement for a future output layout. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --source-file='*CompositorLayerShellZoneTests.cpp' --no-skip`, `./build/tests/lambda_tests --test-case='*layer shell placement*' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorOutputStateTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, and `git diff --check` passed. |
| 2026-06-12 | WM-COMP-26 | Verified | Extended `OutputGeometry` with logical origin and routed `outputGeometryFor()` plus xdg-popup constraints through the selected output layout. Window snap detection, snap targets, maximize, center snap, resize clamps, restored drag, and popup unconstraining now use output bounds with origin instead of assuming `(0,0)`, with non-zero-origin tests preserving current zero-origin behavior. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --source-file='*CompositorWindowGeometryTests.cpp' --no-skip`, `./build/tests/lambda_tests --test-case='*logical output origin*,*logical output center*,*output origin*' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorWindowStateTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*CompositorOutputStateTests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file='*Compositor*Tests.cpp' --no-skip`, `./build/tests/lambda_tests --source-file-exclude='*RuntimeInputTests.cpp' --no-skip`, and `git diff --check` passed. |
| 2026-06-12 | WM-COMP-27 | Verified slice | Documented the repeatable real-app visual validation matrix for Lambda apps, Shell, Firefox/browser, GTK, Qt, `foot`, and mpv; extended `scripts/run-real-app-smoke.sh` with an mpv case; and added `scripts/verify-wm-comp27-matrix.sh` so the matrix, pass/fail criteria, and smoke-runner case list cannot drift silently. `bash -n scripts/run-real-app-smoke.sh`, `bash -n scripts/verify-wm-comp27-matrix.sh`, `scripts/run-real-app-smoke.sh --list`, `scripts/verify-wm-comp27-matrix.sh`, and `git diff --check` passed. |
| 2026-06-12 | WM-COMP-27 | Verified slice | Added guarded compositor lifecycle and trace collection to `scripts/run-real-app-smoke.sh`. The runner can now start `lambda-window-manager` with CPU/KMS/pacing traces, wait for `$XDG_RUNTIME_DIR/lambda-window-manager-display`, run selected real-app cases, scan collected app/compositor logs for fatal/protocol/runtime failures, summarize trace paths, require CPU trace samples for owned-compositor runs, and stop the compositor at exit. The matrix verifier now requires this capability. `bash -n scripts/run-real-app-smoke.sh`, `bash -n scripts/verify-wm-comp27-matrix.sh`, `scripts/run-real-app-smoke.sh --help`, `scripts/run-real-app-smoke.sh --list`, `scripts/run-real-app-smoke.sh --no-build --case shell --probe-only`, `scripts/verify-wm-comp27-matrix.sh`, and `git diff --check` passed. |
| 2026-06-12 | WM-COMP-27 | Verified slice | Added `wayland-info` protocol registry validation to the real-app smoke runner. Owned-compositor smoke runs now save `wayland-info.log` plus `wayland-registry-summary.log` and fail if required globals are missing, while non-owned sessions warn if `wayland-info` is unavailable. The matrix verifier now checks that registry validation remains present. `bash -n scripts/run-real-app-smoke.sh`, `bash -n scripts/verify-wm-comp27-matrix.sh`, `scripts/run-real-app-smoke.sh --help`, `scripts/run-real-app-smoke.sh --list`, `scripts/run-real-app-smoke.sh --no-build --case shell --probe-only`, `scripts/verify-wm-comp27-matrix.sh`, and `git diff --check` passed. |
| 2026-06-12 | WM-COMP-27 | Verified slice | Tightened the registry validation list so it now includes the advertised dmabuf global and the correct `ext_background_effect_manager_v1` global, and extended `scripts/verify-wm-comp27-matrix.sh` to compare the smoke-runner required globals against the `wl_global_create` interfaces in `ServerLifecycle.cpp`. `bash -n scripts/run-real-app-smoke.sh`, `bash -n scripts/verify-wm-comp27-matrix.sh`, `scripts/run-real-app-smoke.sh --list`, `scripts/verify-wm-comp27-matrix.sh`, and `git diff --check` passed. |
| 2026-06-13 | WM-COMP-27 | Verified slice | Extended the real-app smoke runner's GTK case to recognize `meld` and `pavucontrol` fallbacks and updated the documented matrix accordingly. On this machine, the full probe now detects every required case: Lambda Settings, Files, Editor, Terminal, Shell, `foot`, Firefox, `meld` for GTK, and mpv, with only optional Qt still unavailable. `bash -n scripts/run-real-app-smoke.sh`, `bash -n scripts/verify-wm-comp27-matrix.sh`, `scripts/run-real-app-smoke.sh --list`, `scripts/run-real-app-smoke.sh --no-build --case gtk --probe-only`, `scripts/run-real-app-smoke.sh --no-build --probe-only`, `scripts/verify-wm-comp27-matrix.sh`, and `git diff --check` passed. |
| 2026-06-13 | WM-COMP-27 | Verified slice | Added `scripts/audit-wm-comp27-validation-gates.sh` and extended the matrix verifier so the remaining target-hardware gates are explicit and checked. On this machine, tools, required real-app probes, the live Lambda display file, live registry probing, Lambda binaries, and evdev devices are ready; this process is not a TTY, `/dev/uinput` is `0600 root:root`, `sudo -n true` still requires a password, and manual cursor/resize/menu/fullscreen/screenshot/video/idle inspection remains pending. `bash -n scripts/audit-wm-comp27-validation-gates.sh`, `scripts/audit-wm-comp27-validation-gates.sh --help`, `scripts/audit-wm-comp27-validation-gates.sh` with the expected pending-gate exit, `bash -n scripts/verify-wm-comp27-matrix.sh`, `scripts/verify-wm-comp27-matrix.sh`, `scripts/run-real-app-smoke.sh --no-build --probe-only`, and `git diff --check` passed. |
| 2026-06-13 | WM-COMP-27 | Verified slice | Added grouped real-app smoke selection so `--case` can be repeated or comma-separated, then ran a focused live Lambda compositor smoke batch for Settings, Files, Editor, and Terminal against `wayland-0`. The runner preserved explicit Shell launch semantics for `--case shell`, continued to detect every required probe with only optional Qt unavailable, captured registry validation, and found zero fatal/protocol/runtime log matches in the focused live batch at `.debug-logs/real-app-smoke/20260613-185337`. `bash -n scripts/run-real-app-smoke.sh`, `bash -n scripts/verify-wm-comp27-matrix.sh`, `scripts/run-real-app-smoke.sh --help`, `scripts/run-real-app-smoke.sh --no-build --case shell --probe-only`, `scripts/run-real-app-smoke.sh --no-build --case lambda-editor,lambda-terminal --probe-only`, `scripts/run-real-app-smoke.sh --no-build --case lambda-settings --case lambda-files --probe-only`, `scripts/run-real-app-smoke.sh --no-build --seconds 3 --case lambda-settings,lambda-files,lambda-editor,lambda-terminal`, `scripts/verify-wm-comp27-matrix.sh`, and `git diff --check` passed. |
