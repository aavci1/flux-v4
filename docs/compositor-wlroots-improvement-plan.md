...
| P26 | WM-COMP-27 Visual regression and real-app harness | In progress | Added the real-app validation matrix and a drift checker for the smoke runner; repeatable launch/log collection already exists and now includes mpv | `verify-wm-comp27-matrix.sh`, smoke-runner syntax/list checks, plus future trace helper tests | Target hardware visual gate remains |
...
2. In progress: `scripts/run-real-app-smoke.sh` launches clients and collects logs against an already-running compositor; it now includes mpv in the matrix. Starting/stopping the compositor and collecting richer traces remains planned.
