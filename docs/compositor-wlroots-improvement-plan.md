...
| P21 | WM-COMP-22 XDG toplevel configure-state parity | Verified slice | `ack_configure` serial consumption, pending-to-current configure commit, and pending-configure frame-size selection are now explicit tested helpers; remaining consumer audit continues | XDG surface/window-state tests plus compositor suite pass | No manual gate for these helper slices |
...
3. In progress: moved that frame-size decision into a shared helper used by snapshots, top chrome hit testing, and cutout controls hit testing. Remaining renderer/chrome/window-management consumers still need the full audit.
4. In progress: added automated tests for `ack_configure` serial consumption, stale configure pruning, resize-configure detection, pending-to-current configure commit, and pending-configure interactive frame-size selection. Broader resize/maximize/fullscreen/chrome consumer tests remain planned.
