...
| P22 | WM-COMP-23 Seat focus and grab workflow parity | Verified slices | xdg-popup grabs now follow the wlroots popup-grab model for the implemented pointer/keyboard seat paths; unmapped XDG toplevels plus destroyed XDG popup/toplevel and layer-shell roles now clear stale seat focus/order/grab/serial/activation state while deactivating pointer constraints; cursor requests, cursor-shape device surface-destroy lifetime, popup-parent unmap/role-destroy cancellation, non-popup implicit pointer-button grabs including motion focus pinning, popup-grab keyboard-focus pin/restore decisions, keyboard Escape popup-grab stack cancellation, unsupported-touch seat resource handling, stale popup-grab seat-resource cleanup, and data-device/primary-selection device seat-resource cleanup now use tested helpers. Broader live-resource keyboard/menu workflow tests remain planned. | Seat serial, pointer-button grab, popup, keyboard-focus, layer-shell, window-state, pointer-constraint, cursor-shape, global-resource, config, compositor, data-device, and broader feasible tests pass | Firefox app/context menu actions and click-open submenus passed; no manual gate needed for these cleanup slices |
...
15. Planned: add broader automated tests for keyboard grab start/end/cancel and popup/menu focus workflows that require live Wayland resources.
16. Planned: request manual app menu/dock menu validation if runtime grab behavior changes again.
