...
| P5 | WM-COMP-6 DMABUF feedback and buffer lifetime | In progress | Renderer-supported feedback and scanout-preferred tranches are split; next buffer-release lifetime tests | DMABUF validation and buffer release tests where possible | GPU-client/video validation on target hardware |
