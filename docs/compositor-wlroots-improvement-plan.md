...
| P24 | WM-COMP-25 Layer-shell dynamic behavior parity | In progress | Keyboard interactivity and dynamic committed layer/reserved-zone state are covered by tested helpers; remaining output changes, fullscreen panel reservations, focus, output-scale updates, and popup/menu interactions are planned | Layer-shell tests plus compositor suite | Shell dock/topbar/fullscreen validation if placement/focus behavior changes |
...
4. Verified slice: added committed-state coverage for dynamic layer changes and dock reserved-zone anchor/margin aggregation. Remaining fullscreen panel reservations, focus, output-scale updates, and deeper dynamic placement tests are planned.
