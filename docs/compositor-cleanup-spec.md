...
- ✗ `src/Compositor/Wayland/Server.cpp` is ~200 lines: socket, display, dispatch loop, nothing else.
- ✗ `src/Compositor/Wayland/Globals/` has ~17 protocol files, each 100-300 lines.
- ✗ `src/Compositor/main.cpp` is ~150 lines: signal handling, main loop orchestration.
- ✗ `src/Compositor/Config/` holds configuration loading and TOML integration.
- ✗ `src/Compositor/Surface/`, `Window/`, `Input/`, `Chrome/` each hold their respective concerns.
- ✗ `WaylandServer.hpp` is ~80 lines exposing the public interface, behind pimpl.
- ✗ Zero hand-rolled TOML; tomlplusplus dependency added.
- ✗ Zero duplicate destroy-callback patterns; single template.
...
- ✗ All existing demo clients still work.
