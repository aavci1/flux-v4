...
**1. There's no `.app` bundle.** `lambda_add_example` produces a plain Mach-O executable. App Store needs a `.app` directory with `Contents/Info.plist`, `Contents/MacOS/<binary>`, `Contents/Resources/`, signing, entitlements. CMake supports this with `MACOSX_BUNDLE` and `MACOSX_BUNDLE_INFO_PLIST`. ~50 lines of CMake to set up properly. Has to be a separate target, not via `lambda_add_example`, because the bundle needs different paths.
