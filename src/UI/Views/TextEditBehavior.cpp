...
TextEditBehavior::TextEditBehavior(Signal<std::string>& value, TextEditBehaviorOptions const& opts)
  ...
  caret_ = detail::utf8Clamp(s, static_cast<int>(s.size()));
  anchor_ = caret_;
...
std::pair<int, int> TextEditBehavior::orderedSelection() const {
  return detail::orderedSelection(caret_, anchor_);
}
...
void TextEditBehavior::syncBlinkSubscription() {
  bool const want = focused_ && !disabled_;
...
void TextEditBehavior::mutateValue(std::string next, int newCaret, int newAnchor, bool recordUndo,
  ...
  if (recordUndo) {
    ...
    e.caretBefore = caret_;
    e.anchorBefore = anchor_;
    e.valueAfter = next;
  ...
  caret_ = detail::utf8Clamp(next, newCaret);
  anchor_ = detail::utf8Clamp(next, newAnchor);
...
void TextEditBehavior::insertAtCaret(std::string_view insert, bool fromTyping) {
  ...
  auto [i0, i1] = detail::orderedSelection(caret_, anchor_);
...
void TextEditBehavior::deleteSelectionOrChar(bool forward) {
  ...
  int at = detail::utf8Clamp(buf, caret_);
  if (forward) {
...
void TextEditBehavior::copySelection(bool cut) {
  ...
  auto [a, b] = detail::orderedSelection(caret_, anchor_);
...
void TextEditBehavior::moveLineBoundary(bool end, bool extend) {
  std::string const& s = value_->get();
  int const t = end ? lineEndByte(s, caret_) : lineStartByte(s, caret_);
...
void TextEditBehavior::moveWord(int dir, bool extend) {
  std::string const& s = value_->get();
  int c = detail::utf8Clamp(s, caret_);
...
bool TextEditBehavior::handleKey(KeyEvent const& e) {
  ...
  if (e.key == keys::UpArrow) {
    ...
    if (opts_.multiline && opts_.verticalResolver) {
      int const n = opts_.verticalResolver(caret_, -1);
      moveCaretTo(n, shift);
      return true;
    }
    return false;
...
void TextEditBehavior::undo() {
  ...
  caret_ = detail::utf8Clamp(value_->get(), eEntry.caretBefore);
  anchor_ = detail::utf8Clamp(value_->get(), eEntry.anchorBefore);
...
void TextEditBehavior::redo() {
  ...
  caret_ = detail::utf8Clamp(value_->get(), eEntry.caretAfter);
  anchor_ = detail::utf8Clamp(value_->get(), eEntry.anchorAfter);
