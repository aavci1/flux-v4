...
void fillPattern(void* pixels) {
  ...
  for (int y = 0; y < kHeight; ++y) {
    for (int x = 0; x < kWidth; ++x) {
      bool const checker = ((x / 20) + (y / 20)) % 2 == 0;
      std::uint8_t const red = checker ? 235 : 30;
      std::uint8_t const green = static_cast<std::uint8_t>((x * 180) / std::max(1, kWidth - 1) + 35);
      std::uint8_t const blue = static_cast<std::uint8_t>((y * 180) / std::max(1, kHeight - 1) + 35);
...
void fractionalScalePreferredScale(void* data, wp_fractional_scale_v1*, std::uint32_t scale) {
  auto* client = static_cast<DemoClient*>(data);
...
void registryGlobal(void* data, wl_registry* registry, std::uint32_t name, char const* interface,
  ...
  if (std::strcmp(interface, wl_compositor_interface.name) == 0) {
    ...
    client->fractionalScaleManager = static_cast<wp_fractional_scale_manager_v1*>(
...
void destroyClient(DemoClient& client) {
  if (client.fractionalScale) wp_fractional_scale_v1_destroy(client.fractionalScale);
  ...
  if (client.fractionalScaleManager) wp_fractional_scale_manager_v1_destroy(client.fractionalScaleManager);
...
int main() {
  ...
  try {
    ...
    if (!client.compositor || !client.shm || !client.wmBase || !client.fractionalScaleManager) {
      throw std::runtime_error(
          "compositor is missing wl_compositor, wl_shm, xdg_wm_base, or wp_fractional_scale_manager_v1");
    }

    client.surface = wl_compositor_create_surface(client.compositor);
    client.fractionalScale =
        wp_fractional_scale_manager_v1_get_fractional_scale(client.fractionalScaleManager, client.surface);
    wp_fractional_scale_v1_add_listener(client.fractionalScale, &kFractionalScaleListener, &client);
