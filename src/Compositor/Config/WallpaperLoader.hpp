...
public:
  ...
  [[nodiscard]] std::optional<flux::DecodedImageRgba> takePreview();

  /// Returns decoded pixels when a background load finishes. Call from the render thread only.
  [[nodiscard]] std::optional<flux::DecodedImageRgba> takeReady(double* decodeMilliseconds = nullptr);
...
private:
  ...
  std::optional<flux::DecodedImageRgba> previewReady_;
  std::optional<flux::DecodedImageRgba> ready_;
