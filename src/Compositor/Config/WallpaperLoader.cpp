...
void AsyncWallpaperLoader::workerMain(std::string path,
  ...
  std::optional<flux::DecodedImageRgba> decoded =
      readWallpaperCache(sourcePath, maxLongEdge, cacheRoot);
  bool fromCache = decoded.has_value();

  if (!decoded) {
    decoded = flux::decodeImageRgbaFromFile(path);
    if (decoded) {
      auto const preview = flux::downscaleDecodedImageRgba(*decoded, wallpaperPreviewMaxEdge());
      ...
      *decoded = flux::downscaleDecodedImageRgba(std::move(*decoded), maxLongEdge);
...
std::optional<flux::DecodedImageRgba> AsyncWallpaperLoader::takePreview() {
  std::lock_guard const lock(mutex_);
  return std::exchange(previewReady_, std::nullopt);
}

std::optional<flux::DecodedImageRgba> AsyncWallpaperLoader::takeReady(double* decodeMilliseconds) {
  std::lock_guard const lock(mutex_);
  if (!ready_) {
    return std::nullopt;
  }
...
bool tryLoadWallpaperFromCache(CompositorConfigWatchContext& ctx) {
  ...
  std::optional<flux::DecodedImageRgba> decoded = readWallpaperCache(
  ...
  ctx.appliedConfig.wallpaperImage =
      flux::imageFromDecodedRgba(*decoded, ctx.canvas.gpuDevice());
...
bool pollWallpaperPreview(CompositorConfigWatchContext& ctx) {
  ...
  std::optional<flux::DecodedImageRgba> decoded = ctx.wallpaperLoader->takePreview();
  if (!decoded) {
    return false;
  }

  auto const uploadStart = presentation::SteadyClock::now();
  ctx.appliedConfig.wallpaperPreviewImage = flux::imageFromDecodedRgba(*decoded, ctx.canvas.gpuDevice());
...
bool pollWallpaperLoad(CompositorConfigWatchContext& ctx) {
  ...
  double decodeMs = 0.0;
  std::optional<flux::DecodedImageRgba> decoded = ctx.wallpaperLoader->takeReady(&decodeMs);
  if (!decoded) {
    return false;
  }

  auto const uploadStart = presentation::SteadyClock::now();
  ctx.appliedConfig.wallpaperImage = flux::imageFromDecodedRgba(*decoded, ctx.canvas.gpuDevice());
