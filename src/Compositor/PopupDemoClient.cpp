...
void fillParentBuffer(Buffer& buffer) {
  ...
  for (int y = 0; y < buffer.height; ++y) {
    for (int x = 0; x < buffer.width; ++x) {
      ...
      dst[offset + 0u] = 90;
      dst[offset + 1u] = static_cast<std::uint8_t>((y * 220) / std::max(1, buffer.height - 1));
      dst[offset + 2u] = static_cast<std::uint8_t>((x * 220) / std::max(1, buffer.width - 1));
      dst[offset + 3u] = 0xff;
    }
  }
}

void fillPopupBuffer(Buffer& buffer, int hoverRow, bool clicked) {
  auto* dst = static_cast<std::uint8_t*>(buffer.pixels);
  ...
  for (int y = 0; y < buffer.height; ++y) {
    ...
    for (int x = 0; x < buffer.width; ++x) {
      bool const separator = y == rowHeight || y == rowHeight * 2;
      std::size_t const offset = static_cast<std::size_t>(y) * buffer.stride + static_cast<std::size_t>(x) * 4u;
      if (separator) {
        dst[offset + 0u] = 30;
        dst[offset + 1u] = 30;
        ...
        dst[offset + 0u] = static_cast<std::uint8_t>(checker ? 48 : 210);
        dst[offset + 1u] = static_cast<std::uint8_t>(checker ? 190 : 235);
        dst[offset + 2u] = static_cast<std::uint8_t>(checker ? 235 : 80);
      }
      dst[offset + 3u] = 0xff;
    }
  }
...
Buffer createBuffer(wl_shm* shm, char const* name, int width, int height, bool popup) {
  ...
  if (popup) {
    fillPopupBuffer(buffer, -1, false);
  } else {
    fillParentBuffer(buffer);
  }
...
void repaintPopup(DemoClient& client) {
  if (!client.popupSurface || !client.popupBuffer.buffer) return;
  fillPopupBuffer(client.popupBuffer, client.popupHoverRow, client.popupClicked);
...
void pointerEnter(void* data, wl_pointer*, std::uint32_t, wl_surface* surface, wl_fixed_t surfaceX, wl_fixed_t surfaceY) {
  auto* client = static_cast<DemoClient*>(data);
  if (surface != client->popupSurface) return;
  int const rowHeight = std::max(1, kPopupHeight / 3);
  int const row = std::clamp(static_cast<int>(wl_fixed_to_double(surfaceY)) / rowHeight, 0, 2);
...
void pointerMotion(void* data, wl_pointer*, std::uint32_t, wl_fixed_t, wl_fixed_t surfaceY) {
  auto* client = static_cast<DemoClient*>(data);
  if (client->popupHoverRow < 0) return;
  int const rowHeight = std::max(1, kPopupHeight / 3);
  int const row = std::clamp(static_cast<int>(wl_fixed_to_double(surfaceY)) / rowHeight, 0, 2);
...
int main() {
  ...
  try {
    ...
    if (!client.compositor || !client.subcompositor || !client.shm || !client.seat || !client.wmBase) {
      throw std::runtime_error("compositor is missing wl_compositor, wl_subcompositor, wl_shm, wl_seat, or xdg_wm_base");
    }
