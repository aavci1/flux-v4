...
void fillPattern(void* pixels) {
  ...
  for (int y = 0; y < kHeight; ++y) {
    for (int x = 0; x < kWidth; ++x) {
      std::uint8_t const red = static_cast<std::uint8_t>((x * 255) / std::max(1, kWidth - 1));
      std::uint8_t const green = static_cast<std::uint8_t>((y * 255) / std::max(1, kHeight - 1));
      ...
      // WL_SHM_FORMAT_XRGB8888 is little-endian, so memory order is B, G, R, X.
      dst[offset + 0u] = blue;
      dst[offset + 1u] = green;
      dst[offset + 2u] = red;
      dst[offset + 3u] = 0xff;
...
void registryGlobal(void* data, wl_registry* registry, std::uint32_t name, char const* interface,
  ...
  if (std::strcmp(interface, wl_compositor_interface.name) == 0) {
    ...
    client->idleInhibitManager = static_cast<zwp_idle_inhibit_manager_v1*>(
...
void destroyClient(DemoClient& client) {
  if (client.idleInhibitor) zwp_idle_inhibitor_v1_destroy(client.idleInhibitor);
  ...
  if (client.idleInhibitManager) zwp_idle_inhibit_manager_v1_destroy(client.idleInhibitManager);
