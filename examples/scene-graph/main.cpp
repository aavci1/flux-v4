...
TextMeasure layoutText(TextSystem &textSystem, std::string_view text, Font const &font, Color const &color,
    ...
    Size measuredSize = textSystem.measure(text, font, color, maxWidth, options);
...
BuiltNode buildToggle(Theme const &theme, ToggleVisual visual) {
    ...
    if (visual.disabled) {
        trackColor = visual.value ? withAlpha(visual.onColor, 0.35f) : theme.disabledControlBackgroundColor;
    }
    ...
    float const thumbX = visual.value ? visual.trackWidth - visual.thumbInset - thumbSize : visual.thumbInset;
...
BuiltNode buildMetricsRow(TextSystem &textSystem, Theme const &theme, float width) {
    ...
    float const height = std::max(enabled.size.height, std::max(notifications.size.height, connectivity.size.height));
...
std::unique_ptr<SceneNode> buildToggleDemoScene(TextSystem &textSystem, Theme const &theme) {
...
class SceneGraphExampleWindow final : public Window {
  ...
  private:
    void buildRetainedScene(TextSystem &textSystem) {
        m_sceneGraph.setRoot(buildToggleDemoScene(textSystem, m_theme));
    }
