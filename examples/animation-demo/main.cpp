...
struct PlaybackLab : ViewModifiers<PlaybackLab> {
    auto body() const {
        ...
        float clamped = std::clamp(progress.get(), 0.f, 1.f);
        auto previewValue = useState(clamped);
        useEffect([progress, previewValue] {
            previewValue = std::clamp(progress.get(), 0.f, 1.f);
        });
...
struct AmbientLoopLab : ViewModifiers<AmbientLoopLab> {
    auto body() const {
        ...
        ObserverHandle const tickHandle = AnimationClock::instance().subscribe(
            [phase, reducedMotion = theme.reducedMotion, startedAt](AnimationTick const& tick) {
                if (reducedMotion) {
                    phase = 1.f;
                    return;
                }
        ...
        Reactive::onCleanup([tickHandle] {
            AnimationClock::instance().unsubscribe(tickHandle);
        });
