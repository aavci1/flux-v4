...
usage() {
  cat <<EOF
Usage: scripts/audit-wm-comp27-validation-gates.sh
...
lambda_app_gate() {
  ...
  for app in lambda-window-manager lambda-shell lambda-settings lambda-files lambda-editor lambda-terminal; do
    local path="$BUILD_DIR/apps/$app/$app"
    if [[ "$app" == "lambda-window-manager" ]]; then
      path="$BUILD_DIR/apps/lambda-window-manager/lambda-window-manager"
    fi
...
if "$ROOT/scripts/run-real-app-smoke.sh" --no-build --probe-only >"$TMP_DIR/probe.out" 2>"$TMP_DIR/probe.err"; then
...
if [[ -n "$display" ]]; then
  ...
  gate "PENDING" "live-wayland-session" "WAYLAND_DISPLAY=${WAYLAND_DISPLAY} is set, but $display_file is missing; start lambda-window-manager or run scripts/run-real-app-smoke.sh --start-compositor from a real TTY"
  gate "PENDING" "live-registry-probe" "requires the Lambda compositor display file"
else
  gate "PENDING" "live-wayland-session" "start lambda-window-manager or run scripts/run-real-app-smoke.sh --start-compositor from a real TTY"
...
if [[ -x "$ROOT/scripts/run-real-app-smoke.sh" ]]; then
  gate "READY" "real-app-helper" "scripts/run-real-app-smoke.sh"
else
  gate "PENDING" "real-app-helper" "missing scripts/run-real-app-smoke.sh"
fi

if [[ -x "$ROOT/scripts/verify-wm-comp27-matrix.sh" ]]; then
  gate "READY" "matrix-helper" "scripts/verify-wm-comp27-matrix.sh"
else
  gate "PENDING" "matrix-helper" "missing scripts/verify-wm-comp27-matrix.sh"
fi
